import { Award } from '@/types';

export const awards: Award[] = [
  {
    id: 'rsna-award',
    title: 'RSNA 2024 Magna Cum Laude Award',
    description:
      'Patient Privacy Leaks in LLMs After Federated Training on Medical Reports — awarded at RSNA Chicago 2024.',
    year: '2024',
    icon: 'FaTrophy',
  },
  {
    id: 'pet-summit',
    title: 'Privacy-Enhancing Technology Summit Speaker',
    description:
      'Invited speaker at the Privacy-Enhancing Technology Summit, Zurich 2022.',
    year: '2022',
    icon: 'FaMicrophone',
  },
  {
    id: 'ml-summit',
    title: 'World ML Summit Speaker',
    description: 'Speaker at World ML Summit, Dublin 2019.',
    year: '2019',
    icon: 'FaMicrophone',
  },
  {
    id: 'ireland-finalist',
    title: 'Ireland AI & Data Science Student of the Year Finalist',
    description:
      'Award finalist for both Ireland AI Student of the Year and Ireland Data Scientist Student of the Year in 2018.',
    year: '2018',
    icon: 'FaAward',
  },
  {
    id: 'course-mentor',
    title: 'Course Mentor — University of Texas',
    description:
      'Part-time Course Mentor for the AI & ML Postgraduate Diploma at the University of Texas.',
    icon: 'FaChalkboardTeacher',
  },
  {
    id: 'stanford-cert',
    title: 'Stanford University — Machine Learning',
    description: 'Machine Learning certification from Stanford University.',
    icon: 'FaCertificate',
  },
  {
    id: 'cloudera-cert',
    title: 'Cloudera Certified Developer for Hadoop (CCDH)',
    description: 'Certified developer for Apache Hadoop ecosystem.',
    icon: 'FaCertificate',
  },
  {
    id: 'aws-cert',
    title: 'AWS Certified Cloud Developer — Associate',
    description: 'Amazon Web Services Cloud Developer Associate certification.',
    icon: 'FaCertificate',
  },
];
